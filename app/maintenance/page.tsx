...
export default function MaintenancePlan() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {isMemberModalOpen && (
        <div className={`fixed inset-0 z-[1200] flex items-start sm:items-center justify-center px-3 py-5 sm:p-6 overflow-y-auto ${isMemberModalClosing ? 'animate-modal-fade-out' : 'animate-modal-fade-in'}`}>
          <div
            className={`absolute inset-0 bg-black/80 ${isMemberModalClosing ? 'lg:animate-backdrop-unblur' : 'lg:animate-backdrop-blur'}`}
            onClick={closeMemberModal}
          />
          <div className={`relative w-full max-w-2xl max-h-[calc(100dvh-4rem)] sm:max-h-[92vh] overflow-y-auto rounded-2xl sm:rounded-3xl bg-white shadow-[0_30px_80px_-20px_rgba(0,0,0,0.45)] border border-white/40 ${isMemberModalClosing ? 'animate-modal-slide-down' : 'animate-modal-slide-up'}`}>
            {!isMemberSubmitted ? (
              <>
                ...
                <form onSubmit={handleMemberSubmit} className="px-6 py-6 sm:px-8 sm:py-8 space-y-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setMemberForm((prev) => ({ ...prev, phone: formatPhoneInput(e.target.value) }))}
